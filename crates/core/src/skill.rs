//! Strict JSON contracts that turn agent skills into enforceable policy.
...
pub struct SkillContext {
    ...
    pub occurred_at: u64,
    pub state: String,
    pub evidence: HashMap<String, bool>,
}

impl SkillContext {
    pub fn validate(&self) -> Result<(), String> {
        if self.event.is_empty() || self.event.len() > 120 {
            return Err("skill context event must contain 1 to 120 bytes".into());
        }
        if self.action.is_empty() || self.action.len() > 120 {
            return Err("skill context action must contain 1 to 120 bytes".into());
        }
        if self.agent_id.is_empty() || self.agent_id.len() > 128 {
            return Err("skill context agent_id must contain 1 to 128 bytes".into());
        }
        if self.state.len() > 16_384 {
            return Err("skill context state exceeds 16384 bytes".into());
        }
        if self.evidence.len() > MAX_EVIDENCE_ITEMS {
            return Err(format!(
                "skill context has more than {MAX_EVIDENCE_ITEMS} evidence items"
            ));
        }
        if self
            .evidence
            .keys()
            .any(|key| key.is_empty() || key.len() > 120)
        {
            return Err("skill evidence keys must contain 1 to 120 bytes".into());
        }

        let encoded = serde_json::to_vec(self).map_err(|error| error.to_string())?;

        if encoded.len() > MAX_CONTEXT_BYTES {
            return Err(format!("skill context exceeds {MAX_CONTEXT_BYTES} bytes"));
        }
...
pub fn resolve_branch(skill: &Skill, answer: &DecisionAnswer) -> Result<Branch, String> {
    let question = &skill.decision;

    if answer.question_id != question.id {
        return Err(format!(
            "expected answer for {}, got {}",
            question.id, answer.question_id
        ));
    }
    if !answer.confidence.is_finite() || !(0.0..=1.0).contains(&answer.confidence) {
        return Err("decision confidence is outside [0, 1]".into());
    }
    let branch = match (&question.kind, &answer.value) {
        (QuestionType::Choice, DecisionValue::Choice(choice)) => question
            .choice_outcomes
            .iter()
            .find(|mapping| mapping.value == *choice)
            .map(|mapping| mapping.branch)
            .ok_or_else(|| format!("unknown Laya choice {choice}")),
        (QuestionType::Noul, DecisionValue::Noul(probability)) => {
            resolve_numeric(question, *probability, 1.0)
        }
        (QuestionType::Score, DecisionValue::Score(score)) => {
            resolve_numeric(question, *score, score_maximum(question)?)
        }
        _ => Err("Laya decision type does not match the skill question".into()),
    }?;

    if answer.confidence < question.minimum_confidence {
...
fn resolve_numeric(
    ...
    if !value.is_finite() || !(0.0..=maximum).contains(&value) {
        return Err(format!("Laya decision value is outside [0, {maximum}]"));
    }
...
pub fn missing_evidence(skill: &Skill, context: &SkillContext) -> Vec<String> {
    let mut missing = skill
        .predicates
        .required_evidence
        .iter()
        .filter(|key| context.evidence.get(*key) != Some(&true))
        .cloned()
        .collect::<Vec<_>>();
